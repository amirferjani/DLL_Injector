...
async function openCashier(browser,{width,height,phone,name}){
  const context=await browser.newContext({viewport:{width,height},screen:{width,height},deviceScaleFactor:phone?3:1,isMobile:phone,hasTouch:phone,userAgent:phone?iPhoneUa:undefined,serviceWorkers:'allow'});
  ...
  await page.goto(`${baseUrl}/?test=final-v21-${name}`,{waitUntil:'domcontentloaded'});
  ...
  await page.evaluate(()=>[...document.querySelectorAll('.table-button')].find(node=>node.textContent.trim().startsWith('K3'))?.click());
...
async function metrics(page){
  return page.evaluate(()=>{
    ...
    return{
      ...
      hasBrokenLayout:document.documentElement.classList.contains('rk-phone')||document.documentElement.classList.contains('rk-order-v20'),
...
try{
  browser=await chromium.launch({headless:true});
  {
    ...
    const value=await metrics(page);report.metrics.iphone390=value;
    check('Mobiele scrollherstelcode v21 is actief',value.mobileVersion==='21',value.mobileVersion);
    check('Mislukte v19/v20-layout is niet actief',!value.hasBrokenLayout,String(value.hasBrokenLayout));
    check('Oorspronkelijke twee kolommen op smalle iPhone blijven behouden',value.columns===2,`kolommen=${value.columns}`);
    check('Categorieën blijven op hun oorspronkelijke plaats links',value.railParent==='order-main',value.railParent);
    check('Oorspronkelijke Favorieten/Alle producten/Recent-rij blijft zichtbaar',value.tabsDisplay!=='none',value.tabsDisplay);
    check('Productnaam is alleen licht vergroot',px(value.tileName)>=11&&px(value.tileName)<=12,value.tileName);
    check('Prijs is alleen licht vergroot',px(value.tilePrice)>=10&&px(value.tilePrice)<=11,value.tilePrice);
    check('Categorietekst is alleen licht vergroot',px(value.categoryFont)>=9&&px(value.categoryFont)<=10,value.categoryFont);
    check('Bestaande rekeningfuncties blijven aanwezig',value.actionButtons.some(item=>item.id==='backFloorButton')&&value.actionButtons.some(item=>item.id==='requestBillButton')&&value.actionButtons.some(item=>item.id==='moveTableButton')&&value.actionButtons.some(item=>item.text==='Historiek'),JSON.stringify(value.actionButtons));
    ...
    await page.evaluate(()=>window.scrollTo(0,Math.max(1,(document.querySelector('.product-tile')?.getBoundingClientRect().top||0)+window.scrollY-260)));
    ...
    check('Producttik voegt exact één product toe',after.qty===1,`qty=${after.qty}`);
    ...
    check('Historiek blijft voor de Baas aanwezig',await page.locator('.order-history-button').count()===1,'historiek');
    await page.screenshot({path:`${outDir}/v21-final-iphone390.png`,fullPage:true});
    await context.close();
  }
  {
    const{context,page}=await openCashier(browser,{width:708,height:1050,phone:true,name:'iphone708'});
    const value=await metrics(page);report.metrics.iphone708=value;
    check('Brede iPhone behoudt de oorspronkelijke drie kolommen',value.columns===3,`kolommen=${value.columns}`);
    check('Brede iPhone behoudt de oorspronkelijke categorieplaatsing',value.railParent==='order-main',value.railParent);
  ...
  }
  {
    ...
    const value=await metrics(page);report.metrics.desktop=value;
    check('Desktop krijgt geen mobiele herstelcode',value.mobileVersion==='',value.mobileVersion);
    check('Desktop behoudt vijf productkolommen',value.columns===5,`kolommen=${value.columns}`);
  ...
  writeFileSync(`${outDir}/v21-final-report.json`,JSON.stringify(report,null,2));
  writeFileSync(`${outDir}/v21-final-report.md`,['# Registratiekassa v21 — definitief herstel','',`**Resultaat: ${report.passed?'GESLAAGD':'MISLUKT'}**`,'',...report.checks.map(item=>`- ${item.ok?'✅':'❌'} ${item.name}${item.details?` — ${item.details}`:''}`),report.failure?`\n## Fout\n\n\`\`\`text\n${report.failure}\n\`\`\``:''].join('\n'));
